...
class Tgmsp {
	...
	public function activation() {
		...
		if ( version_compare( $wp_version, '3.3.1', '<' ) ) {
			...
			wp_die( printf( __( 'Sorry, but your version of WordPress, <strong>%s</strong>, does not meet Soliloquy\'s required version of <strong>3.3.1</strong> to run properly. The plugin has been deactivated. <a href="%s">Click here to return to the Dashboard</a>', 'soliloquy' ), $wp_version, admin_url() ) );
		}
	...
	public function init() {
		...
		if ( is_admin() ) :
			...
			if ( isset( $soliloquy_license['license'] ) && isset( $soliloquy_license['upgrade'] ) && $soliloquy_license['upgrade'] ) {
				$args = array(
