...
class Tgmsp_Media {
	...
	public function load_media_upload() {
		...
		if ( $this->is_our_context() ) {
			add_action( 'admin_head_media_upload_library_form', array( $this, 'enqueue_scripts' ) );
	...
	/**
	 * Load scripts for the media upload popup and adds appropriate styles for items that
	 * cannot be attached to the slider.
	 *
	 * @since 1.4.0
	 */
	public function enqueue_scripts() {
		...
		wp_register_script( 'soliloquy-media', plugins_url( 'js/media' . $dev . '.js', dirname( __FILE__ ) ), array( 'jquery' ), '1.0.0', true );
		$args = array(
			'green'		=> Tgmsp_Strings::get_instance()->strings['attachment_green'],
			'notice' 	=> Tgmsp_Strings::get_instance()->strings['attachment_notice'],
			'red'		=> Tgmsp_Strings::get_instance()->strings['attachment_red'],
			'white'		=> Tgmsp_Strings::get_instance()->strings['attachment_white'],
			'why'		=> Tgmsp_Strings::get_instance()->strings['attachment_why']
		);
		wp_enqueue_script( 'soliloquy-media' );
		wp_localize_script( 'soliloquy-media', 'soliloquy_media', $args );
		
		/** Register and enqueue styles */
		wp_register_style( 'soliloquy-media', plugins_url( 'css/media' . $dev . '.css', dirname( __FILE__ ) ) );
		wp_enqueue_style( 'soliloquy-media' );
	...
	public function admin_head() {
		...
		?>
		<style type="text/css">#media-items .post_content, #media-items .url, #media-items .align, #media-items .image-size, #media-items .media-blank tr:nth-child(7n), p.media-types label:last-child, #gallery-settings, #gallery-form .savesend input[type="submit"] { display: none !important; }#media-items td.field { vertical-align: middle; }</style>
		<?php
	...
	public function thickbox_context( $translated_text, $source_text, $domain ) {
		...
		if ( 'Insert into Post' == $source_text )
			return Tgmsp_Strings::get_instance()->strings['slider_insert_tb'];
	...
	public function add_image_link( $fields, $attachment ) {
		...
		if ( $this->is_our_context() ) {
			/** Prepare HTML for link type field */
			$types = Tgmsp_Admin::link_types();
			$link_types = '<label for="_soliloquy_uploads[link_type]">' . Tgmsp_Strings::get_instance()->strings['image_link_type'] . '</label>';
			$link_types .= '<select id="attachments[' . $attachment->ID . '][soliloquy_link_type]" class="soliloquy-link-type" name="attachments[' . $attachment->ID . '][soliloquy_link_type]">';
			foreach ( (array) $types as $type => $data )
				$link_types .= '<option value="' . esc_attr( $data['slug'] ) . '"' . selected( $data['slug'], get_post_meta( $attachment->ID, '_soliloquy_image_link_type', true ), false ) . '>' . esc_html( $data['name'] ) . '</option>';
			$link_types .= '</select>';
		
			$fields['soliloquy_link_type'] = apply_filters( 'tgmsp_extra_media_fields_link_type', array(
				'label' => Tgmsp_Strings::get_instance()->strings['image_link_type_title'],
				'input' => 'html',
				'html' 	=> $link_types
			) );
			...
			$fields['soliloquy_video_link'] = apply_filters( 'tgmsp_extra_media_fields_video_link', array(
				'label' => Tgmsp_Strings::get_instance()->strings['image_link_video'],
				'input' => 'text',
				'value' => get_post_meta( $attachment->ID, '_soliloquy_video_link', true )
			) );
	...
	public function save_image_link( $attachment, $post_var ) {
		...
		if ( $this->is_our_context() ) {
			...
			update_post_meta( $attachment['ID'], '_soliloquy_image_link_type', isset( $post_var['soliloquy_link_type'] ) ? preg_replace( '#[^a-z0-9-_]#', '', $post_var['soliloquy_link_type'] ) : '' );
			...
			update_post_meta( $attachment['ID'], '_soliloquy_video_link', isset( $post_var['soliloquy_video_link'] ) ? esc_url( $post_var['soliloquy_video_link'] ) : '' );
