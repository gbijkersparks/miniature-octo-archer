		...
		function wm_modules_meta_fields() {
			...
			$metaFields = array(
				...
				array(
					"type" => "section-open",
					"section-id" => "general",
					"title" => __( 'Icon', 'atlantes_domain_adm' )
				),
					array(
						"type" => "text",
						"id" => $prefix."link",
						"label" => __( 'Custom link', 'atlantes_domain_adm' ),
						"desc" => __( 'If set, the link will be applied on featured image and module title', 'atlantes_domain_adm' ),
						"validate" => "url"
					),
					array(
						"type" => "checkbox",
						"id" => $prefix."type",
						"label" => __( 'Icon box', 'atlantes_domain_adm' ),
						"desc" => __( 'Style this module as icon box', 'atlantes_domain_adm' ),
						"value" => "icon"
					),
					array(
						...
						array(
							"type" => "box",
							"content" => '<h4>' . __( 'Set the predefined icon below or upload your custom one as featured image', 'atlantes_domain_adm' ) . '</h4><p>' . __( 'Predefined icons are high DPI / Retina display ready.', 'atlantes_domain_adm' ) . '</p><p>' . __( 'If you upload a custom icon, note that icon dimensions should be 32&times;32 px (24&times;24 if custom icon background set) in default layout, or 64&times;64 px (48&times;48 if custom icon background set) in centered icon box display layout. For high DPI / Retina displays double the size of the icon (it will be automatically scaled down).', 'atlantes_domain_adm' ) . '<br /><a class="button thickbox button-set-featured-image" href="' . get_admin_url() . 'media-upload.php?post_id=' . $postId . '&type=image&TB_iframe=1">' . __( 'Set featured image', 'atlantes_domain_adm' ) . '</a>' . '</p>' . __( 'Layout can be set in shortcode or in widget parameters when displaying the Content Module.', 'atlantes_domain_adm' ),
						),
						array(
							"type" => "select",
							"id" => $prefix."font-icon",
							"label" => __( 'Predefined icon', 'atlantes_domain_adm' ),
							"desc" => __( 'Select an icon to display with this icon module (the icons are ready for high DPI / Retina displays). This icon will be prioritized over icon uploaded as featured image.', 'atlantes_domain_adm' ),
							"options" => $menuIcons,
							"icons" => true
						),
						array(
							...
							"label" => __( 'Custom icon background', 'atlantes_domain_adm' ),
							"desc" => __( 'Leave empty for no background color', 'atlantes_domain_adm' ),
							"default" => "",
							"validate" => "color"
						),
					array(
